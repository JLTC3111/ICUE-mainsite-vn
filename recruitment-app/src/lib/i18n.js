 ...
 *
 * SCOPE — this file covers the page *chrome*: nav and footer labels, headings,
 * search copy and aria text. The Recruitment content itself lives in its own
 * module so it can be shared (see src/data/ and @icue/faq-content).
 */
...
const lazyLocaleBackend = {
  ...
  read(language, _namespace, done) {
    ...
    if (!load) {
      done(new Error(`Unsupported recruitment locale: ${language}`), false)
      return
    }

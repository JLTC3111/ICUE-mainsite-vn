...
const FINDING_RE = /(kết quả|phát hiện|cho thấy|chỉ ra|đáng chú ý|quan trọng|tác động|xu hướng|results?|findings?|shows?|indicates?|notably|significant|impact|trend|ergebnis|ergebnisse|feststellung|zeigt|zeigen|bedeutsam|auswirkung|résultat|résultats|constat|montre|montrent|indique|notamment|important|résultats?|impact|結果|調査|判明|示して|重要|影響|傾向|결과|조사|밝혀|보여|중요|영향|추세)/iu
const RECOMMENDATION_RE = /(khuyến nghị|đề xuất|cần ưu tiên|cần phải|nên|phải|ưu tiên|giải pháp|recommend|should|must|priority|action required|proposal|empfehl|sollte|muss|priorität|maßnahme|recommand|devrait|doit|priorité|mesure|推奨|必要|すべき|優先|対策|권고|필요|해야|우선|대책)/iu
const RISK_RE = /(rủi ro|cảnh báo|khẩn cấp|nghiêm trọng|thách thức|đe dọa|risk|warning|urgent|critical|challenge|threat|risiko|warnung|dringend|kritisch|herausforderung|risque|alerte|urgent|critique|défi|menace|リスク|警告|緊急|深刻|課題|脅威|위험|경고|긴급|심각|과제|위협)/iu
...
export function detectImportantPhraseRanges(text, { maxRanges = 12 } = {}) {
  ...
  for (const match of segments) {
    ...
    const range = trimmedRange(source, baseStart, baseStart + raw.length)
